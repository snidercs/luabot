...
def create_luajit_headers(version, platform, output_dir):
    include_dir = '3rdparty/include/luajit-2.1' if platform_is_build(platform) else '3rdparty/%s/include/luajit-2.1' % (platform)
    zip_name = f"luajit-{artifact_platform(platform)}-{version}-headers.zip"
...
def create_luajit_libs(version, platform, output_dir):
    """Create luajit-{version}-{platform}.zip with libraries and share files"""
    base_dir = '3rdparty' if platform_is_build(platform) else f'3rdparty/{platform}'
    zip_name = f"luajit-{artifact_platform(platform)}-{version}-{platform}static.zip"
    zip_path = Path(output_dir) / zip_name
        ...
        if lib_dir.exists():
            for file_path in lib_dir.rglob('*'):
                if file_path.is_file() and file_path.suffix in ['.a', '.lib']:
                    ...
                    arcname = os.path.join (top_level_dir, 'lib',  rel_path)
                    if file_path.suffix == '.a':
                        arcname = arcname.replace('.a', f'-{platform}.a')
                    zipf.write(file_path, Path (arcname))
        
        # Add share/lua directory to lua/
        share_lua = base_path / 'share' / 'lua'
        if share_lua.exists():
            for root, dirs, files in os.walk(share_lua):
                for file in files:
                    file_path = Path(root) / file
                    rel_path = file_path.relative_to(share_lua)
                    arcname = Path(top_level_dir) / 'lua' / rel_path
                    zipf.write(file_path, arcname)
        
        # Add share/luajit-2.1 directory to luajit-2.1/
        share_luajit = base_path / 'share' / 'luajit-2.1'
        if share_luajit.exists():
            for root, dirs, files in os.walk(share_luajit):
                for file in files:
                    file_path = Path(root) / file
                    rel_path = file_path.relative_to(share_luajit)
                    arcname = Path(top_level_dir) / 'luajit-2.1' / rel_path
                    zipf.write(file_path, arcname)
    
    # Copy to debug zip (GradleRIO requires debug zips)
    import shutil
...
def create_luabot_headers(version, build_dir, source_dir, output_dir):
    """Create luabot-{version}-headers.zip with headers from build/include and include/"""
    zip_name = f"luabot-{version}-headers.zip"
    zip_path = Path(output_dir) / zip_name
...
def main():
    ...
    generate_pom(args.version, 'util/luabot.pom.in', 'luabot', args.output_dir)

    luajit_version = '2.1'
    for platform in list(set([frc_platform(), 'linuxathena'])):
        create_luajit_headers (luajit_version, platform, args.output_dir)
        create_luajit_libs (luajit_version, platform, args.output_dir)
        artifact_id = f'luajit-{artifact_platform(platform)}'
        generate_pom(luajit_version, 'util/luajit.pom.in', artifact_id, args.output_dir)

    if args.install:
        install_artifact('luabot', args.version, args.output_dir)
        for platform in list(set([frc_platform(), 'linuxathena'])):
            artifact_id = f'luajit-{artifact_platform(platform)}'
            install_artifact (artifact_id, luajit_version, args.output_dir)
